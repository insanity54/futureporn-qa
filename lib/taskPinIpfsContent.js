...
export async function idempotentlyPinIpfsContent(appContext, data, cluster) {
  let results = []
  const cids = [
  ...
  for (const vc of validCids) {
    ...
    if (pinCount < 1) {
      appContext.logger.log({ level: 'info', message: `${vc} is pinned on ${pinCount} cluster peers.` })
...
export async function taskPinIpfsContent(appContext, body, discordClient, cluster) {
  ...
  const pins = await idempotentlyPinIpfsContent(appContext, body, cluster)
  appContext.logger.log({ level: 'info', message: `${JSON.stringify(pins)}` })

  if (pins.length > 0) {
    const discordChannel = discordClient.channels.cache.get(appContext.env.DISCORD_CHATOPS_CHANNEL_ID);
